...
const App = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.data.role !== 'admin') {
        setLoginError('Access Denied: Only Administrator users can access this panel.');
        return;
      }
      ...
      setLoginError('Could not connect to the backend server. Make sure the backend is running.');
    }
  };
  ...
  const fetchCalendarData = async () => {
    if (!token) return;
    try {
      const res = await fetch(`${API_BASE_URL}/calendar-notes?month=${currentMonth}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleSaveNote = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/calendar-notes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteNote = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/calendar-notes/${noteId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
